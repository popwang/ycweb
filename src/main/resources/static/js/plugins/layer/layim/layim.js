...
xxim.expend = function(){
    ...
    if(xxim.layimNode.attr('state') !== '1'){
        ...
        xxim.layimNode.stop().animate({right: -232}, config.aniTime, function(){
...
xxim.userhtml=function(param){
	  var i_user_id	=param.data.i_user_id;	// userid 自增长
	    ...
	    var userhtml=''
	        +'      <p>姓名：'+'<strong>'+v_real_name+'</strong>'+'</p>'
	        +'      <p>部门：'+'<strong>'+v_system_name+'</strong>'+'</p>'
	        +'      <p>性别：'+'<strong>'+v_user_sex+'</strong>'+'</p>'
	        +'      <p>角色：'+'<strong>'+v_type_name+'</strong>'+'</p>'
	        +'      <p>级别：'+'<strong>'+v_positio+'</strong>'+'</p>'
	        +'      <p>电话：'+'<strong>'+v_user_tel+'</strong>'+'</p>'
	        +'      <p>QQ：'+'<strong>'+v_user_qq+'</strong>'+'</p>'
	        +'      <p>微信：'+'<strong>'+v_user_wx+'</strong>'+'</p>'
	        +'';
	    return userhtml;
}
//聊天窗口
    ...
    log.success = function(layero){
       ...
       // layer.setMove();
        ...
        xxim.chatbox.find('.layer_setmin').on('click', function(){
        	 xxim.userlayer=layero;//最小化就全局变量设置弹框的对象 用于点击其他头像恢复弹框
            ...
            node.layimMin.on('click', function(){
                 $(this).hide();
                layero.show();
                xxim.userlayer=null;//最大化就不需要恢复了
             });
             	  ...
             	  if(datas.status === 1){
                      ...
                      +'<ul class="xxim_chatlist">';
                       for(var j = 0; j < item.length; j++){
	                      	...
	                      	if(item[j].v_photo_path==null){
	                      		item[j].v_photo_path=xxim.faceURL(item[j].v_user_sex);
	                      	}        
                           str += '<li data-id="'+ item[j].i_user_id 
                            +'"class="xxim_childnode" type="one"><img src="'+ item[j].v_photo_path +'" class="xxim_oneface"><span class="xxim_onename">'+ item[j].v_real_name +'</span></li>';
                      }
                       str += '</ul></li>';
