...
(function (factory) {
    ...
    function refreshDropMenu($input, data, options) {
        ...
        if (options.showHeader) {
            html.push('<thead><tr>');
            /***niuchen 20170707添加 用于多数据情况下的问题.**/
            if(len>=1000){
         	   html.push('<tr><th>','返回数据过多,只能展示1000条,可输入更准确条件缩小范围!',
                        '' , // 表头第一列记录总数
                        '</th></tr>');
            }else if(len>=1001){
            	alert("开发级错误警告!请开发人员控制数据返回数量.不要超过1000条数据.过多无意义,也会有性能问题.后期扩展成分页!");
            	return;
            }
            /***niuchen 20170707添加 用于多数据情况下的问题.**/
            html.push('<tr>');
    ...
    var defaultOptions = {
        ...
        qingchu: null       // 搜索过滤数据前，对输入关键字作进一步处理方法。注意，应返回字符串
    };
            ...
            return self.each(function() {
                ...
                if ($iClear.length) {
                    $iClear.click(function () {
                        ...
                        setBackground($input, options);
                        //niuchen 追加 201707 增加清除事件
                        $input.trigger('onDeleteValue') ;
                    });
